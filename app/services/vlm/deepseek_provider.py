"""DeepSeek VLM Provider — OpenAI-compatible API via http.client.

Uses ``http.client.HTTPSConnection`` (stdlib, no third-party HTTP deps) to
avoid ``httpx`` / ``httpcore`` TLS handshake issues on macOS with certain
network configurations.  The sync HTTP call is offloaded to a thread via
``asyncio.to_thread`` so the provider remains async-compatible.
"""
...
REQUEST_TIMEOUT = 20  # 秒（含图片上传，比纯文本稍长）
...
def _bgr_to_data_url(bgr: np.ndarray, quality: int = 80) -> str:
...
def _call_api(api_key: str, payload: dict, timeout: float) -> dict:
    """Synchronous HTTP POST to DeepSeek /v1/chat/completions via http.client."""
    ...
    try:
        ...
        if resp.status != 200:
            raise VLMError(f"DeepSeek HTTP {resp.status}: {raw[:300]}")
        return json.loads(raw)
    except VLMError:
        raise
    except Exception as e:
        raise VLMError(f"DeepSeek API call failed: {e}")
    finally:
        conn.close()


class DeepSeekProvider(VLMProvider):
    """DeepSeek VLM Provider (OpenAI-compatible, http.client backend)."""
    ...
    async def assess_scene(
        ...
        *,
        ...
        """Call DeepSeek chat API with a single camera frame.

        The sync ``http.client`` call is offloaded to a thread so the
        event loop is not blocked.
        """
        if not self._api_key:
            raise VLMError("DEEPSEEK_API_KEY not set")
        ...
        data = await asyncio.to_thread(
            _call_api, self._api_key, payload, self._timeout
        )
        ...
        try:
            ...
            raise VLMError(f"DeepSeek response parse failed: {e}\nRaw: {str(data)[:300]}")
